...
- **Estado:** conjunto de pessoas em cada margem e o lado em que está a tocha. Cada pessoa é identificada pelo seu tempo de travessia (1, 2, 5 e 10 minutos).
...
- **`make report`**: Executa `run`, `transitions` e `heuristics` em sequência, produzindo o relatório completo.
...
```
src/
├── models/          # State, Arc (aresta) e Path (caminho na fronteira), BridgeProblem
├── algorithms/      # BaseSearch (procedimento geral e métricas), buscas cegas e informada
└── utils/           # espaço de estados, heurísticas, relatórios e geração do grafo
tests/               # modelagem, algoritmos, heurísticas e propriedades do grafo
```
